...
## Did not end up using (but keep just in case : )

```{r} 
EEall <- merge(EEall,EEchem_dups,by=c("UniqueAccession","ProximateEvent_Type_broad","Type_system_broad"),all=TRUE,sort=TRUE)

EEcount <- EEall %>% group_by(PhysBroadType,ProximateEvent_Type_broad,OrgLevel,Type_system_broad) 
count(EEall['PhysBroadType']=='Temperature' & EEall['ProximateEvent_Type_broad']=='Flood' & EEall['OrgLevel']=='community' & EEall['Type_system_broad']=='freshwater')
```
